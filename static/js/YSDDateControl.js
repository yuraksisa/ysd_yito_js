define(['jquery'], function($){
   ...
   */
  YSDDateControl = function(comboDay, comboMonth, comboYear, hiddenDate, locale) {
  	
    // Creates the model
    var theModel = new YSDDateControlModel(locale);
    ...
    this.setDate = function(date) { /* Sets the date */
  	
  	  if (!isNaN(date)) { /* Be sure it's a valid date */
        theModel.setDate(date);
  	  }
  ...
  YSDDateControlModel = function (locale) {
    ...
    this.checkDaysOfMonth = function() {
      ...
      this.days_of_month = Date.getDaysInMonth(this.year, this.month);
